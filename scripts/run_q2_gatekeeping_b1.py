...
GNN = R / "runs_b1" / "gnn" / "predictions"
LGB = R / "runs_b1" / "lgbm" / "predictions"
LGB_NAIVE = R / "runs_b1" / "naive" / "predictions"
LGB_REP = R / "runs_b1" / "lgbm" / "predictions"
DATA = R / "data_b1"
MORT = Path(r"<ECOTOX_DATA_DIR>\processed\ecotox_mortality_processed.csv")
FROZEN = {"delta": R / "audit" / "delta_primary_frozen_b1.json",                   # per-seed δ [C14]
          "delta_prime": R / "audit" / "delta_prime_frozen_b1.json",              # ensemble δ′ [C16]
          "delta_det": R / "audit" / "delta_det_frozen_b1.json"}                   # det δ_det [C15]
...
def comparison_set():
    comps = _primary_like("primary", "b1_group", gated_stage2="b1_species_cold")
    # confirmatory family REMOVED for B1: no replication/confirmatory partition (structural). §6 exception.
    # [C15] deterministic (LightGBM within-'backbone') TOST: t3a/t3b/t4 vs t2 ; decide with δ_det
    for t in TOST_TIERS:
        comps.append(dict(family="deterministic", backbone="lightgbm", test="TOST", split="b1_group",
    ...
    for bb in GNN_BB + ["lightgbm"]:
        ...
        for trunc in ["t3a_g", "t3a_gf"]:
            for refk in ["t3a", "t2"]:
                comps.append(dict(family=E, subtype="rank", backbone=bb, test="TOST", split="b1_group",
    ...
    for bb in GNN_BB:
        for t in TOST_TIERS:
            for b in ["1-5", "6-20", "21-100", "100+"]:
                comps.append(dict(family=E, subtype="support-bin", backbone=bb, test="TOST", split="b1_group",
    ...
    for bb in GNN_BB:
        for t in TOST_TIERS:
            for g in ["fish", "crusta", "algae"]:
                comps.append(dict(family=E, subtype="tax_group", backbone=bb, test="TOST", split="b1_group",
                                  delta="delta", cand=(bb, GVAR[t]), candbase=(bb, GVAR["t0"]), ref=(bb, GVAR["t2"]),
                                  refbase=(bb, GVAR["t0"]), filt=("tax", g), label=f"{bb}/{t}~t2/{g}"))
    # scaffold + designed-leaky: primary set on other splits
    for sp in ["b1_scaffold", "b1_scaffold_generic", "b1_designed_leaky"]:
    ...
    for bb in GNN_BB:
        for t in TOST_TIERS:
            comps.append(dict(family=E, subtype="cross-backbone", backbone=f"{bb}x lgbm", test="TOST",
                              split="b1_group", delta="delta", cand=(bb, GVAR[t]), candbase=(bb, GVAR["t0"]),
...
def run(execute, dry_check):
    ...
    bins = _count_bins("b1_group"); taxg = _tax_group()
    ...
    if dry_check:
        (R / "runs_b1" / "bootstrap").mkdir(parents=True, exist_ok=True)
        (R / "runs_b1" / "bootstrap" / "comparison_set_enumeration.txt").write_text("\n".join(enum), encoding="utf-8")
        miss = 0
        for c in comps:
    ...
    out = R / "runs_b1" / "bootstrap" / "gatekeeping_results.json"
